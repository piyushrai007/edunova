...
function TeacherDashboard() {
  ...
  const uploadLecture = async (id) => {
    try {
      ...
      const flaskResponse = await fetch('https://api-ilcz.onrender.com/process_video', {
        method: 'POST',
        body: formData,
      });
  ...
  const uploadAttendanceImage = async () => {
    try {
      ...
      const flaskResponse = await fetch('https://api-ilcz.onrender.com/identify-students', {
        method: 'POST',
        body: formData,
      });
      const flaskData = await flaskResponse.json();
